...
class ApiService {
  ...
  static Future<T> _handleResponse<T>(
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        // Account suspended — only clear + redirect if autoLogout is true
        if (autoLogout) {
          _token = '';
          _refreshToken = '';
  ...
  static Future<Map<String, dynamic>> updateProfile({String? fullName}) async {
    return _handleResponse(
      http.put(
        Uri.parse('${AppConfig.baseUrl}/user/profile?${fullName != null ? 'fullName=$fullName' : ''}'),
        headers: _headers,
      ),
      (body) => body['data'] ?? {},
    );
  ...
  /// ✅ NEW: Get current ride status (for polling)
  ...
  /// 🔴 PHASE 2: Get bill details for a completed ride
  static Future<Map<String, dynamic>> getBill(String rideId) async {
    return _handleResponse(
      http.get(
        Uri.parse('${AppConfig.baseUrl}/rides/$rideId/bill'),
        headers: _headers,
      ),
  ...
  /// 🔴 PHASE 2: Confirm bill and trigger payment processing
  static Future<RideData> confirmBill(String rideId) async {
    return _handleResponse(
      http.post(
        Uri.parse('${AppConfig.baseUrl}/rides/$rideId/confirm-bill'),
        headers: _headers,
      ),
      (body) => RideData.fromJson(body['data']),
    );
  ...
  static Future<RideData> setPaymentMethod(String rideId, String method) async {
    return _handleResponse(
      http.post(
        Uri.parse('${AppConfig.baseUrl}/rides/$rideId/payment-method?method=$method'),
        headers: _headers,
      ),
      (body) => RideData.fromJson(body['data']),
    );
  ...
  /// 🔴 PHASE 2C: Get nearby available drivers for user driver discovery
  static Future<List<Map<String, dynamic>>> getNearbyDrivers({
    ...
    return _handleResponse(
      http.get(
        Uri.parse(
          '${AppConfig.baseUrl}/location/nearby-drivers?'
          'latitude=$latitude&longitude=$longitude&'
          'vehicleType=$vehicleType&radius=$radius',
        ),
        headers: _headers,
      ),
      (body) => List<Map<String, dynamic>>.from(
        (body['data'] as List).map((x) => x as Map<String, dynamic>),
      ),
    );
  }

  /// Get user's active ride (if any)
  static Future<RideData?> getActiveRide() async {
    try {
      final response = await http.get(
        Uri.parse('${AppConfig.baseUrl}/rides/user/active'),
        headers: _headers,
      ...
      print('⚠️ Failed to check active ride: $e');
      return null;
    }
  }
...
class RideData {
  ...
  factory RideData.fromJson(Map<String, dynamic> json) {
    return RideData(
      ...
      driverImage: json['driverProfileImageUrl'],  // ✅ Parse driver profile image
